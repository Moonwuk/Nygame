...
- На `time.advanced` — **артиллерийский залп на расстоянии** (`runArtillery`,
  GDD §7.2 «бьёт на расстоянии»): каждый **свободный** флот (не в бою) с
  юнитами-трейтом `artillery` обстреливает **ОДНУ** враждебную цель-флот в радиусе
  `range` (евклид, единицы карты; макс. среди артиллерии флота). **Чистый стэндофф**
  — без ответного огня и без входа в бой; урон `= Σ(artillery count × attack) ×
  часы`, копится по непрерывному отрезку как ПВО. Авто-цель — **ближайший**
  враждебный флот (тай-брейк по id); событие `artillery.fired`. Обнулённая цель →
  `fleet.destroyed`.

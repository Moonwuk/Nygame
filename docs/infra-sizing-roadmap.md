...
2. **Единственный процесс** (1 комната/процесс) — **снято (SV-0.2)**: `MatchRegistry`
   роутит N изолированных матчей/процесс, `LazyMatchRegistry` грузит по запросу и
   гибернирует простаивающие в стор (live-память ∝ активным матчам, risk13). Остаётся
   разнос по >1 процесса/инстанса: **SV-4.1** fan-out; переход на >1 процесс активирует
   Redis-шов `EphemeralStore` (общие пинги/presence/rate-limits/receipts). Пробуждение
   спящего матча к событию при всех офлайн (push/офлайн-victory) — cross-process
   планировщик (pg-boss), тоже SV-4.1.

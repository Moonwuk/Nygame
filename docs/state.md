...
как свипы) — вызывается будущим оркестратором на `match.ended`. **Пока НЕ входит** (нужна
живая сессия из AVA-7): S5 мир (посев `crossTeamStart:'peace'` уже есть в ядре) и S6
эскалация войны (`diplomacy.declare(war)` по таймеру на живом `MatchRoom`). Полный
AvA-цикл дальше: оркестратор сессии (AVA-7), эскалация войны (AVA-8 S5–S6) + лента
(AVA-9), `corporation-wars.md`.

...
packages/shared-core/src/
  ...
  modules/       economy, movement, sector, planetType, combat, construction, army  (+ *.test.ts)
...
1. **Победа и счёт** (`victoryModule`) — условия (доминирование/очки/тайм-аут),
   `match.ended`; сейчас терминального состояния в ядре нет.

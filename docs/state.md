...
> **Гейт:** `pnpm run check` (lint + typecheck + test). **Тесты: 1075 зелёных** (4 skip, 109 файлов).
...
играбельном пути (netserver, REL-4 — в compose по умолчанию ON). Дальше: auth
на прото-пути, OIDC, мультипроцесс.

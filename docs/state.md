...
construction, fleetLaunch])`, тик в реальном времени (скорость ⏸/▶/⏩).
...
- **UI:** карта-галактика на весь экран (процедурная), узлы-планеты по фракциям
  - лейны; **HUD в стиле Iron Order** (латунная лента ресурсов с `+N/h` из
    `netIncome`, герб, деревянная рейка действий, нижняя карточка-досье); **камера
    pan/zoom** (тащить / колесо / pinch / двойной тап-сброс); **адаптив** (мобайл/
    десктоп, media-queries, DPR-чёткость, тач). `netIncome` считает прирост.
- **Стопгап:** прототип захватывает миры авто-спуском+штурмом (`autoEngage`),
  т.к. ручных орбитальных контролов (near/far, bombard) и погрузки армии в UI
  **ещё нет** — это UI-стадия.
...
наземной армии + транспорт (load/unload)**; играбельный прототип с Iron-Order UI.
...
- Прототип: авто-захват-стопгап вместо ручных орбитальных контролов; бомбардировка
  и ПВО есть в ядре, но не выведены в UI; `fleet.launch` — пока прототип-модуль.

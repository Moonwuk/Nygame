...
- **Scheduling:** ✅ a v1 offline scheduler exists — `MatchRoom.tick()`/`msUntilNextEvent()` + a single-process wakeup driver (`clockDriver.ts` in the `packages/server` host; an inline `setTimeout` driver in the prototype host) fire due events with nobody connected. ⚠️ a durable, cross-process wake-up (pg-boss, v2 — NOT Redis/BullMQ) is needed for >1 server process.

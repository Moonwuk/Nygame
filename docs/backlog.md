...
> каркас **action-layer**, multiplayer-slice, **F6 туман на рассылке**, играбельный прототип. 586 тестов зелёные.
...
> гейт зелёный — 586 тестов). Осталось только **C3** (предматчевый выбор/буст).
...
- **PERF-1** ✅ **Сведено и проверено** (ветка devin `engine-optimization`, влита):
  ...
  **Детерминизм сохранён** (golden-RNG + 586 тестов зелёные).
